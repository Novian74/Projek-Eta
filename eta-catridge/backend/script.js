...
// Scirpt ini menjalankan fungsi yang berada di backend

/**
 * Nama Program : Backend Menggunakan JavaScript, HTML dengan Bootstrap 5,
 *                Axios, dan jQuery
 * Deskripsi    : Program ini merupakan script yang digunakan untuk menjalankan seluruh request backend.
 */
...
// 4. Menggunakan jQuery untuk mempermudah manipulasi elemen HTML.
// 5. PrintThis.js digunakan untuk mengonvert data yang telah diambil dari tabel untuk dijadikan pdf yang siap untuk dicetak dalam printer ataupun dalam format pdf

//===========================================
//==         Struktur Folder dan File       ==
//===========================================
// - index.php    : File utama untuk page admin setelah login pada login page.
// - PrintThis.js : Program untuk mengonvert data kasar menjadi pdf
// - Report.html  : Halaman untuk menampilkan report atau histori sesuai dengan tanggal yang dipilih dan dapat didownlaod juga berupa pdf dengan bantuan dependensasi PrintThis,js
// - script.js    : Script untuk menjalankan request dari index.html dan report.html dengan menghubungkannya dengan database laravel menggunakan metode axios
...
function tinta() {
  ...
  axios.get("http://127.0.0.1:8000/api/catridge").then(function (response) {
    // Membuat variabel agar dipermudah
    let catridge = response.data;

    // Menyiapkan tabel cartidge
    let out =
      "<a class='me-4' href='#catridge'>Cartridge</a><a href='#toner'>Toner</a><table class='table' id='cartridge'><thead><tr><th scope='col'>No</th><th scope='col'>Nama Catridge</th><th scope='col'>Warna</th><th scope='col'>Minimum Stok</th><th scope='col'>Stok</th><th></th></tr></thead><tbody>";

    // Memecah array dengan foreach
    catridge.forEach((el, index) => {
      // Variabel untuk membuat nomor mulai dari 1 sampai data terakhir
      let number = index + 1;

      // Menyiapkan isi tabel cartridge
      out += `<tr>
                <td>${number}</td>
    ...
    // Menutup tabel catridge
    out += "</tbody></table>";

    // DOM untuk menampilkan tabel cartridge
    document.querySelector(".cont").innerHTML = out;
  });

  // Mengambil data toner dari database
  axios.get("http://127.0.0.1:8000/api/toner").then(function (response) {
    // Membuat variabel agar dipermudah
    let toner = response.data;

    // Menyiapkan tabel toner
    let out2 =
      "<table class='table' id='toner'><thead><tr><th scope='col'>No</th><th scope='col'>Nama Toner</th><th scope='col'>Warna</th><th scope='col'>Minimum Stok</th><th scope='col'>Stok</th><th></th></tr></thead><tbody>";

    // Memecah array dengan foreach
    toner.forEach((el, index) => {
      // Variabel untuk membuat nomor mulai dari 1 sampai data terakhir
      let number = index + 1;

      // Menyiapkan isi tabel toner
      out2 += `<tr>
                <td>${number}</td>
                <td>${el.catridge_name}</td>
                <td>${el.warna}</td>
    ...
    // DOM untuk menampilkan tabel toner
    document.querySelector(".cont2").innerHTML = out2;
  })
...
function tambahTinta() {
  ...
  axios.get("http://127.0.0.1:8000/api/tintas").then(function (response) {
    ...
    let data = {
      ...
      TC: document.getElementById("tModel").value
    }
...
function ubahTinta() {
  ...
  swal("Berhasil Diubah !", " ", "success").then(tinta())
...
function hapusTinta(id) {
  ...
  swal("Berhasil Dihapus !", " ", "success").then(tinta())
...
function printer() {
  // Menghilangkan tabel yang tertumpuk
  document.querySelector(".cont2").innerHTML = "<h1></h1>";
...
function tambahPrinter() {
  ...
  axios.get("http://127.0.0.1:8000/api/printer").then(function (response) {
    ...
    axios.post("http://127.0.0.1:8000/api/printer", dataP).then(swal("Berhasil Ditambahkan !", " ", "success").then(printer()), printer());
  })
}
...
function hapusPrinter(id) {
  ...
  axios.delete("http://127.0.0.1:8000/api/printer/" + id).then(swal("Berhasil Dihapus !", " ", "success").then(printer()), printer());
}
...
function tambahPrintCat() {
  ...
  swal("Berhasil Menambah Printer + Cartridge !", " ", "success")
...
function hapusPrintCat(idPrCt) {
  ...
  axios.delete("http://127.0.0.1:8000/api/printcat/" + idPrCt).then(swal("Berhasil Dihapus Printer + Cartridge !", " ", "success").then(printCat()), printCat());
}
...
function pelanggan() {
  ...
  // Menghilangkan tombol yang tertumpuk
...
function pending(nota, id) {
  ...
  axios.get("http://127.0.0.1:8000/api/tintas/" + id).then(function (response) {
    ...
    stok.forEach(el => {
      ...
      swal("Barang Ready !", " ", "success").then(booking())
...
function statusR(id) {
  ...
  swal("Sudah Diambil !", " ", "success").then(pickup())
...
function history() {
  ...
  document.querySelector(".btn").innerHTML = `<div class="col-3"><input type="text" id="searchInput" placeholder="Cari..." class="form-control"></div><div class="col ka"><select id="nama" class="form-select kk"><option value="">Nama</option></select></div><div class="col ka"><select id="departemen" class="form-select kk"><option value="">Departemen</option></select></div><div class="col ka"><select id="printer" class="form-select kk"><option value="">Printer</option></select></div><div class="col"><button class="btn btn-success ki" onclick="goToReport()">${btnReport}</button></div>`;
  ...
  axios
    ...
    .then(function (response) {
      ...
      // Memecah data array dengan foreach
      test.forEach((el) => {
        // Menambahkan nilai nama ke dalam Set
        uniqueNames.add(el.nama);
      });

      // Iterasi melalui Set dan membuat opsi baru untuk setiap nama unik
      ...
      // Membuat objek Set untuk menyimpan data unik
      const uniqueDepartemen = new Set();

      // Memecah data array menggunakan foreach
      test.forEach((el) => {
        // Menambahkan departemen ke objek Set
        uniqueDepartemen.add(el.departemen);
      });

      // Mengubah objek Set menjadi array
      const uniqueDepartemenArray = Array.from(uniqueDepartemen);

      // Membuat opsi baru untuk setiap departemen unik
      uniqueDepartemenArray.forEach((departemen) => {
        // Membuat opsi baru
        const option = document.createElement("option");
...
function tgl_kirim() {
  ...
  swal("Berhasil Mengubah !", " ", "success").then(setting())
